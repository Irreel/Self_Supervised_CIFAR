...
_CIFAR10_DATASET_DIR = './datasets/CIFAR10'
_CIFAR100_DATASET_DIR = './datasets/CIFAR100'
# _VOC_DATASET_DIR = './datasets/VOC'
    ...
    def __init__(self, dataset_name, split, random_sized_crop=False,
        ...
        if self.dataset_name=='cifar10':
            ...
            self.data = datasets.__dict__[self.dataset_name.upper()](
                _CIFAR10_DATASET_DIR, train=self.split=='train',
            ...
            self.transform = transforms.Compose(transform)
            self.data = datasets.__dict__[self.dataset_name.upper()](
                _CIFAR100_DATASET_DIR, train=self.split=='train',
                download=True, transform=self.transform)
        else:
...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, transform, unsupervised=False):
...
class DataLoader(object):
    ...
    def get_iterator(self, epoch):
        ...
        dataset = CustomDataset(self.dataset, transform, unsupervised=self.unsupervised)
        ...
        data_loader = torch.utils.data.DataLoader(dataset, batch_size=self.batch_size, 
                                                    ...
                                                    collate_fn=_collate_fun if self.unsupervised else default_collate)
        return data_loader
