...
class Algorithm():
    def __init__(self, opt):
        ...
        if opt['tensorboard']:
            ...
            self.tf = SummaryWriter(os.path.join(opt['exp_dir'],'tensorboard'))
        else: self.tf = None
    ...
    def solve(self, data_loader_train, data_loader_test):
        ...
        if self.tf is not None: self.tf.close()
    ...
    def run_train_epoch(self, data_loader, epoch):
        ...
        disp_step   = int(self.opt['disp_step']) if ('disp_step' in self.opt) else 50
        ...
        for idx, batch in enumerate(tqdm(data_loader(epoch))):
            ...
            if self.tf is not None:
                self.tf.add_scalar("training loss", train_stats_this["loss"], (epoch) * self.bnumber + idx)
